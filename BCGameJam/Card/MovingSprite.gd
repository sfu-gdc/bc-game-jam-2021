...
func _input(event):
	...
	if event is InputEventMouseMotion:
		...
		print("globalPos: %s", global_position)
		print("eventPos: %s ", event.global_position)
		print("mousePos: %s", get_global_mouse_position())
	...
	# add a new object in the level node
		...
		_add_object()
		print("mousePos 2: %s", get_viewport().get_mouse_position())
